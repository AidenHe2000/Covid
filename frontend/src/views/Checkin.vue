      ...
      <v-card>
        ...
        <v-card-text>
          <v-card outlined class="px-6">
            <v-row>
              <v-col v-for="(item, index) in formItems" :key="index" sm="12" md="6" lg="4">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      updateCount() {
        const that = this;
        this.axios.get("http://localhost:8181/patient/getPatientInfo", {params: {
            page: 1,
            size: 0
          }}).then(response => {
          this.totalPatientLength = response.data.totalCount;
        })
          .catch(error => {
            alert('无法连接到服务器，刷新重试。\n' + error.message);
          });

        this.axios.get("http://localhost:8181/prescription/getPrescriptionNumber", {params: {
            page: 1,
            size: 0
          }}).then(response => {
            console.log(response);
          this.totalPrescirptionLength = response.data.data;
        })
          .catch(error => {
            alert('无法连接到服务器，刷新重试。\n' + error.message);
          });

        this.axios.get("http://localhost:8181/diagnosis/getDiagnosisNumber",{params: {
            page: 1,
            size: 0
          }}).then(response => {
          this.totalDiagnosisLength = response.data.data;
        })
          .catch(error => {
            alert('无法连接到服务器，刷新重试。\n' + error.message);
          });
      ...
      checkInPatient() {
        ...
        this.axios.get('http://localhost:8181/patient/createPatient',{params: p})
        .then(response => {
          console.log(response);
