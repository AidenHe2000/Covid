...
if __name__ == '__main__':
    ...
    while i <= size:
        ...
        password_change_date = fake.date_between_dates(date_start=datetime.date(2020, 2, 1),
                                                       date_end=datetime.date(2021, 5, 10))
        registration_ip = fake.ipv4(network=False, address_class=None, private=None)

        t = [user_id, user_name, password, user_email, registration_date, role, user_gender, password_change_date,
             registration_ip]
        lst.append(t)
        i = i + 1
